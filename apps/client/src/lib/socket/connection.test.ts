import { DEFAULT_API_PORT, DEFAULT_SERVER_PORT, type WelcomeMessage } from "@dont-fall/shared";
...
describe("resolveEndpoints", () => {
  ...
  it("points every backend at the host serving the page", () => {
    ...
    // One API origin (ADR 0058) — tracks, assets, auth, and lobbies share it.
    expect(endpoints.apiUrl).toBe(`http://example.test:${DEFAULT_API_PORT}`);
  });

  it("forwards a Playtest Track onto the Match server connection", () => {
    const endpoints = resolveEndpoints("localhost", { trackId: "playtest-abc" });
  ...
  it("connects to the port the lobby broker named, not the fixed default (ADR 0054)", () => {
    const endpoints = resolveEndpoints("localhost", { matchServerPort: 51234 });

    expect(endpoints.matchServerUrl).toBe("ws://localhost:51234/");
    // The API stays on its own fixed port — only the Lobby moved.
