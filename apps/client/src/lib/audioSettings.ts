...
export const AUDIO_CHANNELS = ["master", "effects", "environment", "music"] as const;
...
/** MASTER and MUSIC keep the pane's old mock values; EFFECTS takes IMPACTS & GRABS'. ENVIRONMENT is new, a first guess. */
export const DEFAULT_AUDIO_VOLUMES: AudioVolumes = { master: 78, effects: 92, environment: 70, music: 42 };
...
export const applyAudioVolumes = (
  engine: { setVolume(channel: AudioChannel, gain: number): void } | null,
  volumes: AudioVolumes,
): void => {
  if (!engine) return;
  for (const channel of AUDIO_CHANNELS) engine.setVolume(channel, volumeGain(volumes[channel]));
};
