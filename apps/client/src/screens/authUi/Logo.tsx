...
/** App icon on its own. */
export function LogoMark({ size = 3.6, className, style }: { size?: number; className?: string; style?: CSSProperties }) {
  return (
    <div
...
export default function Logo({ variant = 'horizontal', size = 7.5, mono, chrome, className, style }: LogoProps) {
  if (variant === 'icon') {
    return <LogoMark size={size} {...(className === undefined ? {} : { className })} {...(style === undefined ? {} : { style })} />;
  }
