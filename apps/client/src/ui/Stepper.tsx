...
export default function Stepper({ value, min = 1, max = 9, label, onChange }: StepperProps) {
  ...
  return (
    <div className={s.stepper}>
      <button type="button" className={s.btn} aria-label={name('Fewer')} data-ui-sound="tick" disabled={value <= min} onClick={() => onChange?.(Math.max(min, value - 1))}>
        <svg viewBox="0 0 14 4" aria-hidden="true"><rect width="14" height="4" rx="2" /></svg>
      </button>
      <span className={s.value} data-df-numeric>{value}</span>
      <button type="button" className={[s.btn, s.plus].join(' ')} aria-label={name('More')} data-ui-sound="tick" disabled={value >= max} onClick={() => onChange?.(Math.min(max, value + 1))}>
