...
describe("openDb migrating the pre-ADR-0053 accounts schema", () => {
  ...
  it("adds a nullable friend_code to pre-Friends accounts, preserving rows (M9 ticket 12)", () => {
    // The exact pre-Friends shape by hand — every column except `friend_code`.
    const raw = new Database(dbPath);
    raw.exec(`
      CREATE TABLE accounts (
        id TEXT PRIMARY KEY,
    ...
    const row = db.select().from(accounts).where(eq(accounts.id, "old-id")).get();
    expect(row?.displayName).toBe("Old Bean");
    expect(row?.friendCode).toBeNull();

    // And the migrated column enforces uniqueness like a fresh one.
    db.update(accounts).set({ friendCode: "ABC123" }).where(eq(accounts.id, "old-id")).run();
    db.insert(accounts)
      .values({ id: "new-id", email: "new@example.com", displayName: "New Bean", createdAt: Date.now() })
      .run();
    expect(() =>
      db.update(accounts).set({ friendCode: "ABC123" }).where(eq(accounts.id, "new-id")).run(),
    ).toThrow();
  });
