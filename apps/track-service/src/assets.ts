 ...
 * loader fetches GLB bytes through, instead of per-app `public/` copies
 * that drift silently. Revisions float (latest wins); in-match consistency
 * comes from fetch-once-per-loader, with the documented mid-fetch-edit
 * window — revision-pinned art belongs to the content-pipeline milestone.
 */

export const ASSET_CONTENT_TYPE = "model/gltf-binary";

/** `<moduleId>.glb` and nothing else — the charset leaves no room for traversal, nesting, or suffix games. */
const ASSET_FILE_PATTERN = /^([A-Za-z0-9_-]+)\.glb$/;
...
export const defaultAssetsDir = (): string => join(dirname(fileURLToPath(import.meta.url)), "..", "..", "..", "assets");
...
export const readAssetFile = async (assetsDir: string, fileName: string): Promise<{ bytes: Uint8Array; contentType: string }> => {
  ...
  return { bytes, contentType: ASSET_CONTENT_TYPE };
};
