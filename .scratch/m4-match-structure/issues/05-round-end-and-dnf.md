...
- [x] The Round ends early when every connected Character has qualified, and otherwise when the Time
      Limit reaches zero — both decided by the server, from the same state it already owns (ADR 0040)
- [x] Everyone not qualified when the Round ends is eliminated. A Fall still never eliminates — it
      only costs time through Respawn (CONTEXT.md)
- [x] A disconnect removes the Character and records a DNF; there is no mid-Round rejoin
- [x] The phase advances through round-end into results, carried on the Snapshot like the rest
- [x] The HUD shows how many have qualified out of how many are connected, and tells an eliminated
      Player that they were eliminated
- [x] Manually verified live with two browsers, all three endings: both qualify, the clock expires on
